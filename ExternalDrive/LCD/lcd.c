...
void LCD_ShowTitle(u16 x, u16 y, u8 char_num, u16 PenColor, u16 BackColor)
{
    u8 size = 32;
    u8 temp;
    // 遍历每个字符
    for (u8 num = 0; num < char_num; num++) {
        // 遍历列（32列，每列4字节）
        for (u16 col = 0; col < size; col++) {
            // 遍历当前列的4个字节（对应行0~7、8~15、16~23、24~31）
            for (u8 byte_idx = 0; byte_idx < 4; byte_idx++) {
                // 计算当前字节在title数组中的索引：列×4 + 字节序号
                u16 data_idx = col * 4 + byte_idx;
                if (data_idx >= 128) break; // 防止越界
                temp = title[num][data_idx];

                // 遍历字节的8位（对应列col的8个像素行）
                for (u8 bit_idx = 0; bit_idx < 8; bit_idx++) {
                    // 计算绘制坐标（字符间距2像素，避免重叠）
                    u16 draw_x = x + num * (size + 2) + col;
                    u16 draw_y = y + byte_idx * 8 + bit_idx;

                    // 仅在屏幕范围内绘制
                    if (draw_x < lcddev.width && draw_y < lcddev.height) {
                        if (temp & 0x80) // 最高位为1，画前景色
                            LCD_Fast_DrawPoint(draw_x, draw_y, PenColor);
                        else // 最高位为0，画背景色
                            LCD_Fast_DrawPoint(draw_x, draw_y, BackColor);
                    }
                    temp <<= 1; // 处理下一位
                }
            }
...
void LCD_AnimateTriangleWave(uint16_t StartY, uint16_t WaveLength,
                             uint16_t Amplitude, uint16_t Color, int8_t dir,
                             uint8_t step, uint16_t delay_ms, uint8_t SegmentNum)
{
    uint16_t show_width = 480; 
    uint16_t total_wave_len = WaveLength * (SegmentNum + 2);
    int16_t roll_offset = 0;
    uint16_t wave_min_y = StartY - Amplitude - 2;
    uint16_t wave_max_y = StartY + 2;

    #define SCREEN_HEIGHT 480 
    if (wave_max_y >= SCREEN_HEIGHT) {
        wave_max_y = SCREEN_HEIGHT - 1;
    }

    if (StartY >= SCREEN_HEIGHT) {
        StartY = SCREEN_HEIGHT - Amplitude - 5; 
    }

    while (1) 
    {

        LCD_Fill(0, wave_min_y, show_width - 1, wave_max_y, BLACK);
        uint16_t x_screen; // 屏幕X坐标（0~show_width-1）
        for (x_screen = 0; x_screen < show_width; x_screen++) {
            uint16_t x_wave   = (x_screen + roll_offset) % total_wave_len;
            uint16_t x_in_seg = x_wave % WaveLength;
            uint16_t half_len = WaveLength / 2;
            uint16_t y;

            if (x_in_seg < half_len) {
                y = StartY - (Amplitude * x_in_seg) / half_len;
            } else {
                y = StartY - Amplitude + (Amplitude * (x_in_seg - half_len)) / half_len;
            }

        }

        roll_offset += dir * step;
        if (roll_offset < 0) roll_offset += total_wave_len;
        if (roll_offset >= total_wave_len) roll_offset -= total_wave_len;

        LCD_delayms(delay_ms);
    }
}